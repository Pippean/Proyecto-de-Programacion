package com.mycompany.losentussi;

import java.awt.Graphics2D;
import java.awt.geom.Path2D;

/**
 *
 * @author Pipe, Juano, Lázaro
 */
public class Mayusculas {

    public Mayusculas() {

    }

    public void dibujarA(Graphics2D g2, int x, int y, boolean bandera, boolean tilde, boolean negrita, boolean subrayado, boolean cursiva,float tamaño, boolean EspejoY) {
        Path2D.Double A = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                A.moveTo(((55 * EY) + x)*tamaño, (53 + y)*tamaño);
                A.curveTo(((0 * EY) + x)*tamaño, (53 + y)*tamaño, ((0 * EY) + x)*tamaño, (143 + y)*tamaño, ((25 * EY) + x)*tamaño, (140 + y)*tamaño);
                A.moveTo(((55 * EY) + x)*tamaño, (53 + y)*tamaño);
                A.curveTo(((80 * EY) + x)*tamaño, (53 + y)*tamaño, ((80 * EY) + x)*tamaño, (143 + y)*tamaño, ((25 * EY) + x)*tamaño, (140 + y)*tamaño);
                A.moveTo(((66 * EY) + x)*tamaño, (110 + y)*tamaño);
                A.curveTo(((67 * EY) + x)*tamaño, (110 + y)*tamaño, ((70 * EY) + x)*tamaño, (143 + y)*tamaño, ((100 * EY) + x)*tamaño, (143 + y)*tamaño);
            } else {
                A.moveTo(((40 * EY) + x)*tamaño, (55 + y)*tamaño);
                A.curveTo(((0 * EY) + x)*tamaño, (55 + y)*tamaño, ((0 * EY) + x)*tamaño, (143 + y)*tamaño, ((40 * EY) + x)*tamaño, (143 + y)*tamaño);
                A.moveTo(((40 * EY) + x)*tamaño, (55 + y)*tamaño);
                A.curveTo(((80 * EY) + x)*tamaño, (55 + y)*tamaño, ((80 * EY) + x)*tamaño, (143 + y)*tamaño, ((40 * EY) + x)*tamaño, (143 + y)*tamaño);
                A.moveTo(((70 * EY) + x)*tamaño, (110 + y)*tamaño);
                A.curveTo(((70 * EY) + x)*tamaño, (110 + y)*tamaño, ((70 * EY) + x)*tamaño, (143 + y)*tamaño, ((100 * EY) + x)*tamaño, (143 + y)*tamaño);
            }
            if (subrayado == true) {
                A.moveTo(((0 * EY) + x)*tamaño, (153 + y)*tamaño);
                A.lineTo(((110 * EY) + x)*tamaño, (153 + y)*tamaño);
            }
            if (tilde == true) {
                A.moveTo(((40 * EY) + x)*tamaño, (50 + y)*tamaño);
                A.lineTo(((60 * EY) + x)*tamaño, (30 + y)*tamaño);
                if (bandera == true) {
                    g2.fillOval((40 * EY) + x, 50 + y, 5, 5);
                    g2.fillOval((60 * EY) + x, 30 + y, 5, 5);
                }
            }
            x += 1;

        }

        if (bandera == true) {
            g2.fillOval((40 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((0 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((0 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((40 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((80 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((80 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((70 * EY) + x, 110 + y, 5, 5);
            g2.fillOval((70 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((100 * EY) + x, 143 + y, 5, 5);
        }
        g2.draw(A);
    }

    public void dibujarB(Graphics2D g2, int x, int y, boolean bandera, boolean negrita, boolean subrayado, boolean cursiva,float tamaño, boolean EspejoY) {
        Path2D.Double B = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                B.moveTo(((0 * EY) + x)*tamaño, (70 + y)*tamaño);
                B.curveTo(((0 * EY) + x)*tamaño, (70 + y)*tamaño, ((15 * EY) + x)*tamaño, (70 + y)*tamaño, ((20 * EY) + x)*tamaño, (55 + y)*tamaño);
                B.moveTo(((20 * EY) + x)*tamaño, (55 + y)*tamaño);
                B.curveTo(((20 * EY) + x)*tamaño, (55 + y)*tamaño, ((10 * EY) + x)*tamaño, (143 + y)*tamaño, ((10 * EY) + x)*tamaño, (143 + y)*tamaño);
                B.moveTo(((20 * EY) + x)*tamaño, (55 + y)*tamaño);
                B.curveTo(((70 * EY) + x)*tamaño, (55 + y)*tamaño, ((60 * EY) + x)*tamaño, (100 + y)*tamaño, ((30 * EY) + x)*tamaño, (100 + y)*tamaño);
                B.moveTo(((30 * EY) + x)*tamaño, (100 + y)*tamaño);
                B.curveTo(((50 * EY) + x)*tamaño, (100 + y)*tamaño, ((60 * EY) + x)*tamaño, (153 + y)*tamaño, ((0 * EY) + x)*tamaño, (143 + y)*tamaño);
            } else {
                B.moveTo(((0 * EY) + x)*tamaño, (70 + y)*tamaño);
                B.curveTo(((0 * EY) + x)*tamaño, (70 + y)*tamaño, ((10 * EY) + x)*tamaño, (65 + y)*tamaño, ((10 * EY) + x)*tamaño, (55 + y)*tamaño);
                B.moveTo(((10 * EY) + x)*tamaño, (55 + y)*tamaño);
                B.curveTo(((10 * EY) + x)*tamaño, (55 + y)*tamaño, ((10 * EY) + x)*tamaño, (143 + y)*tamaño, ((10 * EY) + x)*tamaño, (143 + y)*tamaño);
                B.moveTo(((10 * EY) + x)*tamaño, (55 + y)*tamaño);
                B.curveTo(((60 * EY) + x)*tamaño, (45 + y)*tamaño, ((60 * EY) + x)*tamaño, (100 + y)*tamaño, ((30 * EY) + x)*tamaño, (100 + y)*tamaño);
                B.moveTo(((30 * EY) + x)*tamaño, (100 + y)*tamaño);
                B.curveTo(((60 * EY) + x)*tamaño, (100 + y)*tamaño, ((60 * EY) + x)*tamaño, (153 + y)*tamaño, ((10 * EY) + x)*tamaño, (143 + y)*tamaño);
            }
            if (subrayado == true) {
                B.moveTo(((0 * EY) + x)*tamaño, (153 + y)*tamaño);
                B.lineTo(((70 * EY) + x)*tamaño, (153 + y)*tamaño);
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((0 * EY) + x, 70 + y, 5, 5);
            g2.fillOval((10 * EY) + x, 65 + y, 5, 5);
            g2.fillOval((10 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((10 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((60 * EY) + x, 45 + y, 5, 5);
            g2.fillOval((60 * EY) + x, 100 + y, 5, 5);
            g2.fillOval((30 * EY) + x, 100 + y, 5, 5);
            g2.fillOval((60 * EY) + x, 153 + y, 5, 5);
        }

        g2.draw(B);

    }

    public void dibujarC(Graphics2D g2, int x, int y, boolean bandera, boolean negrita, boolean subrayado, boolean cursiva,float tamaño, boolean EspejoY) {
        Path2D.Double C = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                C.moveTo(((80 * EY) + x)*tamaño, (70 + y)*tamaño);
                C.curveTo(((0 * EY) + x)*tamaño, (45 + y)*tamaño, ((0 * EY) + x)*tamaño, (170 + y)*tamaño, ((70 * EY) + x)*tamaño, (143 + y)*tamaño);
            } else {
                C.moveTo(((70 * EY) + x)*tamaño, (55 + y)*tamaño);
                C.curveTo(((0 * EY) + x)*tamaño, (45 + y)*tamaño, ((0 * EY) + x)*tamaño, (153 + y)*tamaño, ((70 * EY) + x)*tamaño, (143 + y)*tamaño);
            }
            if (subrayado == true) {
                C.moveTo(((0 * EY) + x)*tamaño, (153 + y)*tamaño);
                C.lineTo(((80 * EY) + x)*tamaño, (153 + y)*tamaño);
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((70 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((0 * EY) + x, 45 + y, 5, 5);
            g2.fillOval((0 * EY) + x, 154 + y, 5, 5);
            g2.fillOval((70 * EY) + x, 143 + y, 5, 5);
        }

        g2.draw(C);
    }

    public void dibujarD(Graphics2D g2, int x, int y, boolean bandera, boolean negrita, boolean subrayado, boolean cursiva,float tamaño, boolean EspejoY) {
        Path2D.Double D = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                D.moveTo(((75 * EY) + x)*tamaño, (65 + y)*tamaño);
                D.curveTo(((60 * EY) + x)*tamaño, (65 + y)*tamaño, ((60 * EY) + x)*tamaño, (120 + y)*tamaño, ((52 * EY) + x)*tamaño, (130 + y)*tamaño);
                D.moveTo(((52 * EY) + x)*tamaño, (130 + y)*tamaño);
                D.curveTo(((32 * EY) + x)*tamaño, (160 + y)*tamaño, ((0 * EY) + x)*tamaño, (128 + y)*tamaño, ((30 * EY) + x)*tamaño, (130 + y)*tamaño);
                D.moveTo(((30 * EY) + x)*tamaño, (130 + y)*tamaño);
                D.curveTo(((40 * EY) + x)*tamaño, (145 + y)*tamaño, ((100 * EY) + x)*tamaño, (155 + y)*tamaño, ((110 * EY) + x)*tamaño, (100 + y)*tamaño);
                D.moveTo(((110 * EY) + x)*tamaño, (100 + y)*tamaño);
                D.curveTo(((110 * EY) + x)*tamaño, (60 + y)*tamaño, ((65 * EY) + x)*tamaño, (45 + y)*tamaño, ((50 * EY) + x)*tamaño, (60 + y)*tamaño);
            } else {
                D.moveTo(((55 * EY) + x)*tamaño, (55 + y)*tamaño);
                D.curveTo(((45 * EY) + x)*tamaño, (65 + y)*tamaño, ((60*EY) + x)*tamaño, (120 + y)*tamaño, ((47*EY) + x)*tamaño, (130 + y)*tamaño);
                D.moveTo(((47 * EY) + x)*tamaño, (130 + y)*tamaño);
                D.curveTo(((30 * EY) + x)*tamaño, (160 + y)*tamaño, ((0 * EY) + x)*tamaño, (128 + y)*tamaño, ((30 * EY) + x)*tamaño, (130 + y)*tamaño);
                D.moveTo(((30 * EY) + x)*tamaño, (130 + y)*tamaño);
                D.curveTo(((40 * EY) + x)*tamaño, (145 + y)*tamaño, ((100 * EY) + x)*tamaño, (155 + y)*tamaño, ((110 * EY) + x)*tamaño, (100 + y)*tamaño);
                D.moveTo(((110 * EY) + x)*tamaño, (100 + y)*tamaño);
                D.curveTo(((110 * EY) + x)*tamaño, (60 + y)*tamaño, ((75 * EY) + x)*tamaño, (45 + y)*tamaño, ((30 * EY) + x)*tamaño, (55 + y)*tamaño);
            }
            if (subrayado == true) {
                D.moveTo(((0 * EY) + x)*tamaño, (153 + y)*tamaño);
                D.lineTo(((120 * EY) + x)*tamaño, (153 + y)*tamaño);
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((55 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((45 * EY) + x, 65 + y, 5, 5);
            g2.fillOval((60 * EY) + x, 120 + y, 5, 5);
            g2.fillOval((47 * EY) + x, 130 + y, 5, 5);
            g2.fillOval((30 * EY) + x, 160 + y, 5, 5);
            g2.fillOval((0 * EY) + x, 128 + y, 5, 5);
            g2.fillOval((30 * EY) + x, 130 + y, 5, 5);
            g2.fillOval((40 * EY) + x, 145 + y, 5, 5);
            g2.fillOval((100 * EY) + x, 155 + y, 5, 5);
            g2.fillOval((110 * EY) + x, 100 + y, 5, 5);
            g2.fillOval((110 * EY) + x, 60 + y, 5, 5);
            g2.fillOval((75 * EY) + x, 45 + y, 5, 5);
            g2.fillOval((30 * EY) + x, 55 + y, 5, 5);
        }

        g2.draw(D);
    }

    public void dibujarE(Graphics2D g2, int x, int y, boolean bandera, boolean tilde, boolean negrita, boolean subrayado, boolean cursiva,float tamaño, boolean EspejoY) {
        Path2D.Double E = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                E.moveTo(((20 * EY) + x)*tamaño, (60 + y)*tamaño);
                E.curveTo(((30 * EY) + x)*tamaño, (50 + y)*tamaño, ((50 * EY) + x)*tamaño, (65 + y)*tamaño, ((80 * EY) + x)*tamaño, (55 + y)*tamaño);
                E.moveTo(((50 * EY) + x)*tamaño, (57 + y)*tamaño);
                E.curveTo(((50 * EY) + x)*tamaño, (55 + y)*tamaño, ((50 * EY) + x)*tamaño, (133 + y)*tamaño, ((0 * EY) + x)*tamaño, (143 + y)*tamaño);
                E.moveTo(((40 * EY) + x)*tamaño, (100 + y)*tamaño);
                E.curveTo(((40 * EY) + x)*tamaño, (100 + y)*tamaño, ((60 * EY) + x)*tamaño, (100 + y)*tamaño, ((55 * EY) + x)*tamaño, (100 + y)*tamaño);
                E.moveTo(((0 * EY) + x)*tamaño, (143 + y)*tamaño);
                E.curveTo(((30 * EY) + x)*tamaño, (130 + y)*tamaño, ((50 * EY) + x)*tamaño, (145 + y)*tamaño, ((60 * EY) + x)*tamaño, (143 + y)*tamaño);
            } else {
                E.moveTo(((10 * EY) + x)*tamaño, (60 + y)*tamaño);
                E.curveTo(((30 * EY) + x)*tamaño, (50 + y)*tamaño, ((50 * EY) + x)*tamaño, (65 + y)*tamaño, ((70 * EY) + x)*tamaño, (55 + y)*tamaño);
                E.moveTo(((40 * EY) + x)*tamaño, (57 + y)*tamaño);
                E.curveTo(((40 * EY) + x)*tamaño, (55 + y)*tamaño, ((50 * EY) + x)*tamaño, (133 + y)*tamaño, ((10 * EY) + x)*tamaño, (143 + y)*tamaño);
                E.moveTo(((40 * EY) + x)*tamaño, (100 + y)*tamaño);
                E.curveTo(((40 * EY) + x)*tamaño, (100 + y)*tamaño, ((60 * EY) + x)*tamaño, (100 + y)*tamaño, ((55 * EY) + x)*tamaño, (100 + y)*tamaño);
                E.moveTo(((10 * EY) + x)*tamaño, (143 + y)*tamaño);
                E.curveTo(((30 * EY) + x)*tamaño, (130 + y)*tamaño, ((50 * EY) + x)*tamaño, (145 + y)*tamaño, ((70 * EY) + x)*tamaño, (143 + y)*tamaño);
            }
            if (subrayado == true) {
                E.moveTo(((0 * EY) + x)*tamaño, (153 + y)*tamaño);
                E.lineTo(((80 * EY) + x)*tamaño, (153 + y)*tamaño);
            }
            if (tilde == true) {
                E.moveTo(((40 * EY) + x)*tamaño, (50 + y)*tamaño);
                E.lineTo(((60 * EY) + x)*tamaño, (30 + y)*tamaño);
                if (bandera == true) {
                    g2.fillOval((40 * EY) + x, 50 + y, 5, 5);
                    g2.fillOval((60 * EY) + x, 30 + y, 5, 5);
                }
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((10 * EY) + x, 60 + y, 5, 5);
            g2.fillOval((30 * EY) + x, 50 + y, 5, 5);
            g2.fillOval((50 * EY) + x, 65 + y, 5, 5);
            g2.fillOval((70 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((40 * EY) + x, 57 + y, 5, 5);
            g2.fillOval((50 * EY) + x, 130 + y, 5, 5);
            g2.fillOval((10 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((40 * EY) + x, 100 + y, 5, 5);
            g2.fillOval((60 * EY) + x, 100 + y, 5, 5);
            g2.fillOval((55 * EY) + x, 100 + y, 5, 5);
            g2.fillOval((10 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((30 * EY) + x, 130 + y, 5, 5);
            g2.fillOval((50 * EY) + x, 145 + y, 5, 5);
            g2.fillOval((70 * EY) + x, 143 + y, 5, 5);
        }

        g2.draw(E);
    }

    public void dibujarF(Graphics2D g2, int x, int y, boolean bandera, boolean negrita, boolean subrayado, boolean cursiva,float tamaño, boolean EspejoY) {
        Path2D.Double F = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                F.moveTo(((20 * EY) + x)*tamaño, (60 + y)*tamaño);
                F.curveTo(((30 * EY) + x)*tamaño, (50 + y)*tamaño, ((50 * EY) + x)*tamaño, (65 + y)*tamaño, ((80 * EY) + x)*tamaño, (55 + y)*tamaño);
                F.moveTo(((50 * EY) + x)*tamaño, (57 + y)*tamaño);
                F.curveTo(((50 * EY) + x)*tamaño, (55 + y)*tamaño, ((50 * EY) + x)*tamaño, (133 + y)*tamaño, ((0 * EY) + x)*tamaño, (143 + y)*tamaño);
                F.moveTo(((30 * EY) + x)*tamaño, (100 + y)*tamaño);
                F.curveTo(((30 * EY) + x)*tamaño, (100 + y)*tamaño, ((50 * EY) + x)*tamaño, (100 + y)*tamaño, ((65 * EY) + x)*tamaño, (100 + y)*tamaño);
            } else {
                F.moveTo(((10 * EY) + x)*tamaño, (60 + y)*tamaño);
                F.curveTo(((30 * EY) + x)*tamaño, (50 + y)*tamaño, ((50 * EY) + x)*tamaño, (65 + y)*tamaño, ((70 * EY) + x)*tamaño, (55 + y)*tamaño);
                F.moveTo(((40 * EY) + x)*tamaño, (57 + y)*tamaño);
                F.curveTo(((40 * EY) + x)*tamaño, (55 + y)*tamaño, ((50 * EY) + x)*tamaño, (133 + y)*tamaño, ((10 * EY) + x)*tamaño, (143 + y)*tamaño);
                F.moveTo(((20 * EY) + x)*tamaño, (100 + y)*tamaño);
                F.curveTo(((20 * EY) + x)*tamaño, (100 + y)*tamaño, ((40 * EY) + x)*tamaño, (100 + y)*tamaño, ((55 * EY) + x)*tamaño, (100 + y)*tamaño);
            }
            if (subrayado == true) {
                F.moveTo(((0 * EY) + x)*tamaño, (153 + y)*tamaño);
                F.lineTo(((80 * EY) + x)*tamaño, (153 + y)*tamaño);
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((10 * EY) + x, 60 + y, 5, 5);
            g2.fillOval((30 * EY) + x, 50 + y, 5, 5);
            g2.fillOval((50 * EY) + x, 65 + y, 5, 5);
            g2.fillOval((70 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((40 * EY) + x, 57 + y, 5, 5);
            g2.fillOval((50 * EY) + x, 133 + y, 5, 5);
            g2.fillOval((10 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((20 * EY) + x, 100 + y, 5, 5);
            g2.fillOval((40 * EY) + x, 100 + y, 5, 5);
            g2.fillOval((55 * EY) + x, 100 + y, 5, 5);
        }
        g2.draw(F);
    }

    public void dibujarG(Graphics2D g2, int x, int y, boolean bandera, boolean negrita, boolean subrayado, boolean cursiva,float tamaño, boolean EspejoY) {
        Path2D.Double G = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                G.moveTo(((75 * EY) + x)*tamaño, (65 + y)*tamaño);
                G.curveTo(((0 * EY) + x)*tamaño, (55 + y)*tamaño, ((0 * EY) + x)*tamaño, (153 + y)*tamaño, ((40 * EY) + x)*tamaño, (143 + y)*tamaño);
                G.moveTo(((40 * EY) + x)*tamaño, (143 + y)*tamaño);
                G.curveTo(((60 * EY) + x)*tamaño, (143 + y)*tamaño, ((70 * EY) + x)*tamaño, (110 + y)*tamaño, ((80 * EY) + x)*tamaño, (100 + y)*tamaño);
                G.moveTo(((80 * EY) + x)*tamaño, (100 + y)*tamaño);
                G.curveTo(((80 * EY) + x)*tamaño, (100 + y)*tamaño, ((80 * EY) + x)*tamaño, (180 + y)*tamaño, ((50 * EY) + x)*tamaño, (180 + y)*tamaño);
                G.moveTo(((50 * EY) + x)*tamaño, (180 + y)*tamaño);
                G.curveTo(((35 * EY) + x)*tamaño, (183 + y)*tamaño, ((20 * EY) + x)*tamaño, (180 + y)*tamaño, ((50 * EY) + x)*tamaño, (160 + y)*tamaño);
                G.moveTo(((50 * EY) + x)*tamaño, (160 + y)*tamaño);
                G.curveTo(((50 * EY) + x)*tamaño, (160 + y)*tamaño, ((90 * EY) + x)*tamaño, (140 + y)*tamaño, ((100 * EY) + x)*tamaño, (143 + y)*tamaño);
            } else {
                G.moveTo(((60 * EY) + x)*tamaño, (65 + y)*tamaño);
                G.curveTo(((0 * EY) + x)*tamaño, (55 + y)*tamaño, ((0 * EY) + x)*tamaño, (153 + y)*tamaño, ((40 * EY) + x)*tamaño, (143 + y)*tamaño);
                G.moveTo(((40 * EY) + x)*tamaño, (143 + y)*tamaño);
                G.curveTo(((60 * EY) + x)*tamaño, (143 + y)*tamaño, ((70 * EY) + x)*tamaño, (110 + y)*tamaño, ((70 * EY) + x)*tamaño, (100 + y)*tamaño);
                G.moveTo(((70 * EY) + x)*tamaño, (100 + y)*tamaño);
                G.curveTo(((70 * EY) + x)*tamaño, (100 + y)*tamaño, ((80 * EY) + x)*tamaño, (180 + y)*tamaño, ((55 * EY) + x)*tamaño, (180 + y)*tamaño);
                G.moveTo(((55 * EY) + x)*tamaño, (180 + y)*tamaño);
                G.curveTo(((40 * EY) + x)*tamaño, (185 + y)*tamaño, ((15 * EY) + x)*tamaño, (180 + y)*tamaño, ((50 * EY) + x)*tamaño, (160 + y)*tamaño);
                G.moveTo(((50 * EY) + x)*tamaño, (160 + y)*tamaño);
                G.curveTo(((50 * EY) + x)*tamaño, (160 + y)*tamaño, ((90 * EY) + x)*tamaño, (140 + y)*tamaño, ((100 * EY) + x)*tamaño, (143 + y)*tamaño);
            }
            if (subrayado == true) {
                G.moveTo(((0 * EY) + x)*tamaño, (153 + y)*tamaño);
                G.lineTo(((120 * EY) + x)*tamaño, (153 + y)*tamaño);
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((60 * EY) + x, 65 + y, 5, 5);
            g2.fillOval((0 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((0 * EY) + x, 153 + y, 5, 5);
            g2.fillOval((40 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((60 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((70 * EY) + x, 110 + y, 5, 5);
            g2.fillOval((70 * EY) + x, 100 + y, 5, 5);
            g2.fillOval((80 * EY) + x, 180 + y, 5, 5);
            g2.fillOval((55 * EY) + x, 180 + y, 5, 5);
            g2.fillOval((40 * EY) + x, 185 + y, 5, 5);
            g2.fillOval((15 * EY) + x, 180 + y, 5, 5);
            g2.fillOval((50 * EY) + x, 160 + y, 5, 5);
            g2.fillOval((90 * EY) + x, 140 + y, 5, 5);
            g2.fillOval((100 * EY) + x, 143 + y, 5, 5);
        }
        g2.draw(G);
    }

    public void dibujarH(Graphics2D g2, int x, int y, boolean bandera, boolean negrita, boolean subrayado, boolean cursiva, boolean EspejoY) {
        Path2D.Double H = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                H.moveTo((10 * EY) + x, 55 + y);
                H.curveTo((10 * EY) + x, 60 + y, (40 * EY) + x, 60 + y, (40 * EY) + x, 55 + y);
                H.moveTo((40 * EY) + x, 55 + y);
                H.curveTo((50 * EY) + x, 170 + y, (0 * EY) + x, 170 + y, (0 * EY) + x, 140 + y);
                H.moveTo((0 * EY) + x, 140 + y);
                H.curveTo((0 * EY) + x, 120 + y, (60 * EY) + x, 80 + y, (60 * EY) + x, 80 + y);
                H.moveTo((60 * EY) + x, 80 + y);
                H.curveTo((90 * EY) + x, 70 + y, (70 * EY) + x, 40 + y, (60 * EY) + x, 60 + y);
                H.moveTo((60 * EY) + x, 60 + y);
                H.curveTo((60 * EY) + x, 150 + y, (80 * EY) + x, 143 + y, (90 * EY) + x, 143 + y);
            } else {
                H.moveTo((0 * EY) + x, 55 + y);
                H.curveTo((0 * EY) + x, 60 + y, (30 * EY) + x, 60 + y, (30 * EY) + x, 55 + y);
                H.moveTo((30 * EY) + x, 55 + y);
                H.curveTo((50 * EY) + x, 170 + y, (0 * EY) + x, 170 + y, (0 * EY) + x, 140 + y);
                H.moveTo((0 * EY) + x, 140 + y);
                H.curveTo((0 * EY) + x, 120 + y, (60 * EY) + x, 80 + y, (60 * EY) + x, 80 + y);
                H.moveTo((60 * EY) + x, 80 + y);
                H.curveTo((80 * EY) + x, 70 + y, (60 * EY) + x, 40 + y, (50 * EY) + x, 60 + y);
                H.moveTo((50 * EY) + x, 60 + y);
                H.curveTo((50 * EY) + x, 150 + y, (70 * EY) + x, 143 + y, (80 * EY) + x, 143 + y);
            }
            if (subrayado == true) {
                H.moveTo((0 * EY) + x, 153 + y);
                H.lineTo((110 * EY) + x, 153 + y);
            }
            x += 1;
        }
        if (bandera == true) {
            H.moveTo((0 * EY) + x, 55 + y);
            H.curveTo((0 * EY) + x, 60 + y, (30 * EY) + x, 60 + y, (30 * EY) + x, 55 + y);
            H.moveTo((30 * EY) + x, 55 + y);
            H.curveTo((50 * EY) + x, 170 + y, (0 * EY) + x, 170 + y, (0 * EY) + x, 140 + y);
            H.moveTo((0 * EY) + x, 140 + y);
            H.curveTo((0 * EY) + x, 120 + y, (60 * EY) + x, 80 + y, (60 * EY) + x, 80 + y);
            H.moveTo((60 * EY) + x, 80 + y);
            H.curveTo((80 * EY) + x, 70 + y, (60 * EY) + x, 40 + y, (50 * EY) + x, 60 + y);
            H.moveTo((50 * EY) + x, 60 + y);
            H.curveTo((50 * EY) + x, 150 + y, (70 * EY) + x, 143 + y, (80 * EY) + x, 143 + y);
        }
        g2.draw(H);
    }

    public void dibujarI(Graphics2D g2, int x, int y, boolean bandera, boolean tilde, boolean negrita, boolean subrayado, boolean cursiva, boolean EspejoY) {
        Path2D.Double I = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                I.moveTo((0 * EY) + x, 55 + y);
                I.curveTo((0 * EY) + x, 65 + y, (40 * EY) + x, 65 + y, (50 * EY) + x, 55 + y);
                I.moveTo((50 * EY) + x, 55 + y);
                I.curveTo((40 * EY) + x, 170 + y, (0 * EY) + x, 170 + y, (0 * EY) + x, 130 + y);
            } else {
                I.moveTo((0 * EY) + x, 55 + y);
                I.curveTo((0 * EY) + x, 65 + y, (40 * EY) + x, 65 + y, (50 * EY) + x, 55 + y);
                I.moveTo((50 * EY) + x, 55 + y);
                I.curveTo((70 * EY) + x, 170 + y, (0 * EY) + x, 170 + y, (0 * EY) + x, 130 + y);
            }
            if (subrayado == true) {
                I.moveTo((0 * EY) + x, 153 + y);
                I.lineTo((70 * EY) + x, 153 + y);
            }
            if (tilde == true) {
                I.moveTo((25 * EY) + x, 50 + y);
                I.lineTo((45 * EY) + x, 30 + y);
                if (bandera == true) {
                    g2.fillOval((40 * EY) + x, 50 + y, 5, 5);
                    g2.fillOval((60 * EY) + x, 30 + y, 5, 5);
                }
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((0 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((0 * EY) + x, 65 + y, 5, 5);
            g2.fillOval((40 * EY) + x, 65 + y, 5, 5);
            g2.fillOval((50 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((70 * EY) + x, 170 + y, 5, 5);
            g2.fillOval((0 * EY) + x, 170 + y, 5, 5);
            g2.fillOval((0 * EY) + x, 130 + y, 5, 5);
        }
        g2.draw(I);
    }

    public void dibujarJ(Graphics2D g2, int x, int y, boolean bandera, boolean negrita, boolean subrayado, boolean cursiva, boolean EspejoY) {
        Path2D.Double J = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                J.moveTo((10 * EY) + x, 100 + y);
                J.curveTo((20 * EY) + x, 110 + y, (40 * EY) + x, 75 + y, (40 * EY) + x, 75 + y);
                J.moveTo((40 * EY) + x, 75 + y);
                J.curveTo((30 * EY) + x, 170 + y, (0 * EY) + x, 190 + y, (0 * EY) + x, 180 + y);
                J.moveTo((0 * EY) + x, 180 + y);
                J.curveTo((0 * EY) + x, 140 + y, (60 * EY) + x, 143 + y, (60 * EY) + x, 143 + y);
            } else {
                J.moveTo((10 * EY) + x, 110 + y);
                J.curveTo((20 * EY) + x, 110 + y, (40 * EY) + x, 75 + y, (40 * EY) + x, 75 + y);
                J.moveTo((40 * EY) + x, 75 + y);
                J.curveTo((60 * EY) + x, 170 + y, (10 * EY) + x, 190 + y, (10 * EY) + x, 180 + y);
                J.moveTo((10 * EY) + x, 180 + y);
                J.curveTo((10 * EY) + x, 140 + y, (60 * EY) + x, 143 + y, (60 * EY) + x, 143 + y);
            }
            if (subrayado == true) {
                J.moveTo((0 * EY) + x, 153 + y);
                J.lineTo((70 * EY) + x, 153 + y);
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((10 * EY) + x, 110 + y, 5, 5);
            g2.fillOval((20 * EY) + x, 110 + y, 5, 5);
            g2.fillOval((40 * EY) + x, 75 + y, 5, 5);
            g2.fillOval((40 * EY) + x, 75 + y, 5, 5);
            g2.fillOval((60 * EY) + x, 170 + y, 5, 5);
            g2.fillOval((10 * EY) + x, 190 + y, 5, 5);
            g2.fillOval((10 * EY) + x, 180 + y, 5, 5);
            g2.fillOval((10 * EY) + x, 140 + y, 5, 5);
            g2.fillOval((60 * EY) + x, 143 + y, 5, 5);
        }
        g2.draw(J);
    }

    public void dibujarK(Graphics2D g2, int x, int y, boolean bandera, boolean negrita, boolean subrayado, boolean cursiva, boolean EspejoY) {
        Path2D.Double K = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                K.moveTo((40 * EY) + x, 65 + y);
                K.curveTo((15 * EY) + x, 68 + y, (10 * EY) + x, 60 + y, (20 * EY) + x, 57 + y);
                K.moveTo((20 * EY) + x, 57 + y);
                K.curveTo((30 * EY) + x, 52 + y, (40 * EY) + x, 60 + y, (65 * EY) + x, 55 + y);
                K.moveTo((65 * EY) + x, 55 + y);
                K.curveTo((35 * EY) + x, 65 + y, (60 * EY) + x, 110 + y, (37 * EY) + x, 130 + y);
                K.moveTo((37 * EY) + x, 130 + y);
                K.curveTo((20 * EY) + x, 160 + y, (0 * EY) + x, 128 + y, (20 * EY) + x, 130 + y);
                K.moveTo((100 * EY) + x, 55 + y);
                K.curveTo((65 * EY) + x, 53 + y, (80 * EY) + x, 90 + y, (52 * EY) + x, 103 + y);
                K.moveTo((52 * EY) + x, 103 + y);
                K.curveTo((80 * EY) + x, 103 + y, (52 * EY) + x, 143 + y, (100 * EY) + x, 143 + y);
            } else {
                K.moveTo((30 * EY) + x, 65 + y);
                K.curveTo((5 * EY) + x, 68 + y, (0 * EY) + x, 60 + y, (10 * EY) + x, 57 + y);
                K.moveTo((10 * EY) + x, 57 + y);
                K.curveTo((20 * EY) + x, 52 + y, (30 * EY) + x, 60 + y, (55 * EY) + x, 55 + y);
                K.moveTo((55 * EY) + x, 55 + y);
                K.curveTo((45 * EY) + x, 65 + y, (60 * EY) + x, 110 + y, (47 * EY) + x, 130 + y);
                K.moveTo((47 * EY) + x, 130 + y);
                K.curveTo((30 * EY) + x, 160 + y, (0 * EY) + x, 128 + y, (30 * EY) + x, 130 + y);
                K.moveTo((100 * EY) + x, 55 + y);
                K.curveTo((65 * EY) + x, 53 + y, (80 * EY) + x, 90 + y, (52 * EY) + x, 103 + y);
                K.moveTo((52 * EY) + x, 103 + y);
                K.curveTo((80 * EY) + x, 103 + y, (52 * EY) + x, 143 + y, (100 * EY) + x, 143 + y);
            }
            if (subrayado == true) {
                K.moveTo((0 * EY) + x, 153 + y);
                K.lineTo((110 * EY) + x, 153 + y);
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((30 * EY) + x, 65 + y, 5, 5);
            g2.fillOval((5 * EY) + x, 68 + y, 5, 5);
            g2.fillOval((0 * EY) + x, 60 + y, 5, 5);
            g2.fillOval((10 * EY) + x, 57 + y, 5, 5);
            g2.fillOval((20 * EY) + x, 52 + y, 5, 5);
            g2.fillOval((30 * EY) + x, 60 + y, 5, 5);
            g2.fillOval((55 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((45 * EY) + x, 65 + y, 5, 5);
            g2.fillOval((60 * EY) + x, 110 + y, 5, 5);
            g2.fillOval((47 * EY) + x, 130 + y, 5, 5);
            g2.fillOval((30 * EY) + x, 160 + y, 5, 5);
            g2.fillOval((0 * EY) + x, 128 + y, 5, 5);
            g2.fillOval((30 * EY) + x, 130 + y, 5, 5);
            g2.fillOval((100 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((65 * EY) + x, 53 + y, 5, 5);
            g2.fillOval((80 * EY) + x, 90 + y, 5, 5);
            g2.fillOval((52 * EY) + x, 103 + y, 5, 5);
            g2.fillOval((80 * EY) + x, 103 + y, 5, 5);
            g2.fillOval((52 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((100 * EY) + x, 143 + y, 5, 5);
        }
        g2.draw(K);
    }

    public void dibujarL(Graphics2D g2, int x, int y, boolean bandera, boolean negrita, boolean subrayado, boolean cursiva, boolean EspejoY) {
        Path2D.Double L = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                L.moveTo((30 * EY) + x, 65 + y);
                L.curveTo((5 * EY) + x, 68 + y, (0 * EY) + x, 60 + y, (10 * EY) + x, 57 + y);
                L.moveTo((10 * EY) + x, 57 + y);
                L.curveTo((20 * EY) + x, 52 + y, (30 * EY) + x, 60 + y, (55 * EY) + x, 55 + y);
                L.moveTo((55 * EY) + x, 55 + y);
                L.curveTo((40 * EY) + x, 65 + y, (50 * EY) + x, 110 + y, (25 * EY) + x, 130 + y);
                L.moveTo((25 * EY) + x, 130 + y);
                L.curveTo((-5 * EY) + x, 160 + y, (0 * EY) + x, 128 + y, (20 * EY) + x, 130 + y);
                L.moveTo((20 * EY) + x, 130 + y);
                L.curveTo((45 * EY) + x, 130 + y, (55 * EY) + x, 145 + y, (80 * EY) + x, 143 + y);
            } else {
                L.moveTo((30 * EY) + x, 65 + y);
                L.curveTo((5 * EY) + x, 68 + y, (0 * EY) + x, 60 + y, (10 * EY) + x, 57 + y);
                L.moveTo((10 * EY) + x, 57 + y);
                L.curveTo((20 * EY) + x, 52 + y, (30 * EY) + x, 60 + y, (55 * EY) + x, 55 + y);
                L.moveTo((55 * EY) + x, 55 + y);
                L.curveTo((45 * EY) + x, 65 + y, (60 * EY) + x, 110 + y, (47 * EY) + x, 130 + y);
                L.moveTo((47 * EY) + x, 130 + y);
                L.curveTo((30 * EY) + x, 160 + y, (0 * EY) + x, 128 + y, (30 * EY) + x, 130 + y);
                L.moveTo((30 * EY) + x, 130 + y);
                L.curveTo((45 * EY) + x, 130 + y, (55 * EY) + x, 145 + y, (100 * EY) + x, 143 + y);
            }
            if (subrayado == true) {
                L.moveTo((0 * EY) + x, 153 + y);
                L.lineTo((110 * EY) + x, 153 + y);
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((30 * EY) + x, 65 + y, 5, 5);
            g2.fillOval((5 * EY) + x, 68 + y, 5, 5);
            g2.fillOval((0 * EY) + x, 60 + y, 5, 5);
            g2.fillOval((10 * EY) + x, 57 + y, 5, 5);
            g2.fillOval((20 * EY) + x, 52 + y, 5, 5);
            g2.fillOval((30 * EY) + x, 60 + y, 5, 5);
            g2.fillOval((55 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((45 * EY) + x, 65 + y, 5, 5);
            g2.fillOval((60 * EY) + x, 110 + y, 5, 5);
            g2.fillOval((47 * EY) + x, 130 + y, 5, 5);
            g2.fillOval((30 * EY) + x, 160 + y, 5, 5);
            g2.fillOval((0 * EY) + x, 128 + y, 5, 5);
            g2.fillOval((30 * EY) + x, 130 + y, 5, 5);
            g2.fillOval((30 * EY) + x, 130 + y, 5, 5);
            g2.fillOval((45 * EY) + x, 130 + y, 5, 5);
            g2.fillOval((55 * EY) + x, 145 + y, 5, 5);
            g2.fillOval((100 * EY) + x, 143 + y, 5, 5);
        }
        g2.draw(L);
    }

    public void dibujarM(Graphics2D g2, int x, int y, boolean bandera, boolean negrita, boolean subrayado, boolean cursiva, boolean EspejoY) {
        Path2D.Double M = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                M.moveTo((55 * EY) + x, 55 + y);
                M.curveTo((45 * EY) + x, 65 + y, (45 * EY) + x, 110 + y, (27 * EY) + x, 130 + y);
                M.moveTo((27 * EY) + x, 130 + y);
                M.curveTo((10 * EY) + x, 160 + y, (-20 * EY) + x, 128 + y, (10 * EY) + x, 130 + y);
                M.moveTo((55 * EY) + x, 55 + y);
                M.curveTo((48 * EY) + x, 55 + y, (52 * EY) + x, 143 + y, (55 * EY) + x, 143 + y);
                M.moveTo((55 * EY) + x, 143 + y);
                M.curveTo((62 * EY) + x, 143 + y, (85 * EY) + x, 55 + y, (95 * EY) + x, 55 + y);
                M.moveTo((95 * EY) + x, 55 + y);
                M.curveTo((85 * EY) + x, 65 + y, (75 * EY) + x, 143 + y, (100 * EY) + x, 143 + y);
                M.moveTo((100 * EY) + x, 143 + y);
                M.curveTo((122 * EY) + x, 146 + y, (126 * EY) + x, 126 + y, (110 * EY) + x, 130 + y);
            } else {
                M.moveTo((25 * EY) + x, 55 + y);
                M.curveTo((25 * EY) + x, 65 + y, (40 * EY) + x, 110 + y, (27 * EY) + x, 130 + y);
                M.moveTo((27 * EY) + x, 130 + y);
                M.curveTo((10 * EY) + x, 160 + y, (-20 * EY) + x, 128 + y, (10 * EY) + x, 130 + y);
                M.moveTo((25 * EY) + x, 55 + y);
                M.curveTo((25 * EY) + x, 55 + y, (62 * EY) + x, 143 + y, (62 * EY) + x, 143 + y);
                M.moveTo((62 * EY) + x, 143 + y);
                M.curveTo((62 * EY) + x, 143 + y, (85 * EY) + x, 55 + y, (85 * EY) + x, 55 + y);
                M.moveTo((85 * EY) + x, 55 + y);
                M.curveTo((85 * EY) + x, 65 + y, (75 * EY) + x, 143 + y, (100 * EY) + x, 143 + y);
                M.moveTo((100 * EY) + x, 143 + y);
                M.curveTo((122 * EY) + x, 146 + y, (126 * EY) + x, 126 + y, (110 * EY) + x, 130 + y);
            }
            if (subrayado == true) {
                M.moveTo((0 * EY) + x, 153 + y);
                M.lineTo((130 * EY) + x, 153 + y);
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((25 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((25 * EY) + x, 65 + y, 5, 5);
            g2.fillOval((40 * EY) + x, 110 + y, 5, 5);
            g2.fillOval((27 * EY) + x, 130 + y, 5, 5);
            g2.fillOval((10 * EY) + x, 160 + y, 5, 5);
            g2.fillOval((10 * EY) + x, 130 + y, 5, 5);
            g2.fillOval((25 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((62 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((85 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((85 * EY) + x, 65 + y, 5, 5);
            g2.fillOval((75 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((100 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((122 * EY) + x, 146 + y, 5, 5);
            g2.fillOval((126 * EY) + x, 126 + y, 5, 5);
            g2.fillOval((110 * EY) + x, 130 + y, 5, 5);
        }
        g2.draw(M);
    }

    public void dibujarN(Graphics2D g2, int x, int y, boolean bandera, boolean negrita, boolean subrayado, boolean cursiva, boolean EspejoY) {
        Path2D.Double N = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                N.moveTo((45 * EY) + x, 55 + y);
                N.curveTo((45 * EY) + x, 65 + y, (40 * EY) + x, 110 + y, (27 * EY) + x, 130 + y);
                N.moveTo((27 * EY) + x, 130 + y);
                N.curveTo((10 * EY) + x, 160 + y, -20 + x, 128 + y, 10 + x, 130 + y);
                N.moveTo((45 * EY) + x, 55 + y);
                N.curveTo((35 * EY) + x, 65 + y, (50 * EY) + x, 110 + y, (80 * EY) + x, 143 + y);
                N.moveTo((80 * EY) + x, 143 + y);
                N.curveTo((85 * EY) + x, 120 + y, (90 * EY) + x, 55 + y, (130 * EY) + x, 55 + y);
            } else {
                N.moveTo((25 * EY) + x, 55 + y);
                N.curveTo((25 * EY) + x, 65 + y, (40 * EY) + x, 110 + y, (27 * EY) + x, 130 + y);
                N.moveTo((27 * EY) + x, 130 + y);
                N.curveTo((10 * EY) + x, 160 + y, (-20 * EY) + x, 128 + y, (10 * EY) + x, 130 + y);
                N.moveTo((25 * EY) + x, 55 + y);
                N.curveTo((25 * EY) + x, 65 + y, (40 * EY) + x, 110 + y, (80 * EY) + x, 143 + y);
                N.moveTo((80 * EY) + x, 143 + y);
                N.curveTo((85 * EY) + x, 120 + y, (80 * EY) + x, 55 + y, (120 * EY) + x, 55 + y);
            }
            if (subrayado == true) {
                N.moveTo((0 * EY) + x, 153 + y);
                N.lineTo((130 * EY) + x, 153 + y);
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((25 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((25 * EY) + x, 65 + y, 5, 5);
            g2.fillOval((40 * EY) + x, 110 + y, 5, 5);
            g2.fillOval((27 * EY) + x, 130 + y, 5, 5);
            g2.fillOval((10 * EY) + x, 160 + y, 5, 5);
            g2.fillOval((10 * EY) + x, 130 + y, 5, 5);
            g2.fillOval((25 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((25 * EY) + x, 65 + y, 5, 5);
            g2.fillOval((40 * EY) + x, 110 + y, 5, 5);
            g2.fillOval((80 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((85 * EY) + x, 120 + y, 5, 5);
            g2.fillOval((80 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((120 * EY) + x, 55 + y, 5, 5);
        }
        g2.draw(N);
    }

    public void dibujarÑ(Graphics2D g2, int x, int y, boolean bandera, boolean negrita, boolean subrayado, boolean cursiva, boolean EspejoY) {
        Path2D.Double Ñ = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                Ñ.moveTo((45 * EY) + x, 55 + y);
                Ñ.curveTo((45 * EY) + x, 65 + y, (40 * EY) + x, 110 + y, (27 * EY) + x, 130 + y);
                Ñ.moveTo((27 * EY) + x, 130 + y);
                Ñ.curveTo((10 * EY) + x, 160 + y, -20 + x, 128 + y, 10 + x, 130 + y);
                Ñ.moveTo((45 * EY) + x, 55 + y);
                Ñ.curveTo((35 * EY) + x, 65 + y, (50 * EY) + x, 110 + y, (80 * EY) + x, 143 + y);
                Ñ.moveTo((80 * EY) + x, 143 + y);
                Ñ.curveTo((85 * EY) + x, 120 + y, (90 * EY) + x, 55 + y, (130 * EY) + x, 55 + y);
                Ñ.moveTo((35 * EY) + x, 40 + y);
                Ñ.curveTo((45 * EY) + x, 25 + y, (75 * EY) + x, 50 + y, (115 * EY) + x, 40 + y);
            } else {
                Ñ.moveTo((25 * EY) + x, 55 + y);
                Ñ.curveTo((25 * EY) + x, 65 + y, (40 * EY) + x, 110 + y, (27 * EY) + x, 130 + y);
                Ñ.moveTo((27 * EY) + x, 130 + y);
                Ñ.curveTo((10 * EY) + x, 160 + y, (-20 * EY) + x, 128 + y, (10 * EY) + x, 130 + y);
                Ñ.moveTo((25 * EY) + x, 55 + y);
                Ñ.curveTo((25 * EY) + x, 65 + y, (40 * EY) + x, 110 + y, (80 * EY) + x, 143 + y);
                Ñ.moveTo((80 * EY) + x, 143 + y);
                Ñ.curveTo((85 * EY) + x, 120 + y, (80 * EY) + x, 55 + y, (120 * EY) + x, 55 + y);
                Ñ.moveTo((25 * EY) + x, 40 + y);
                Ñ.curveTo((45 * EY) + x, 25 + y, (75 * EY) + x, 50 + y, (115 * EY) + x, 40 + y);
            }
            if (subrayado == true) {
                Ñ.moveTo((0 * EY) + x, 153 + y);
                Ñ.lineTo((130 * EY) + x, 153 + y);
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((25 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((25 * EY) + x, 65 + y, 5, 5);
            g2.fillOval((40 * EY) + x, 110 + y, 5, 5);
            g2.fillOval((27 * EY) + x, 130 + y, 5, 5);
            g2.fillOval((10 * EY) + x, 160 + y, 5, 5);
            g2.fillOval((10 * EY) + x, 130 + y, 5, 5);
            g2.fillOval((25 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((25 * EY) + x, 65 + y, 5, 5);
            g2.fillOval((40 * EY) + x, 110 + y, 5, 5);
            g2.fillOval((80 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((85 * EY) + x, 120 + y, 5, 5);
            g2.fillOval((80 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((120 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((25 * EY) + x, 40 + y, 5, 5);
            g2.fillOval((45 * EY) + x, 25 + y, 5, 5);
            g2.fillOval((75 * EY) + x, 50 + y, 5, 5);
            g2.fillOval((115 * EY) + x, 40 + y, 5, 5);
        }
        g2.draw(Ñ);
    }

    public void dibujarO(Graphics2D g2, int x, int y, boolean bandera, boolean tilde, boolean negrita, boolean subrayado, boolean cursiva, boolean EspejoY) {
        Path2D.Double O = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                O.moveTo((40 * EY) + x, 143 + y);
                O.curveTo((0 * EY) + x, 143 + y, (10 * EY) + x, 55 + y, (35 * EY) + x, 55 + y);
                O.moveTo((40 * EY) + x, 143 + y);
                O.curveTo((80 * EY) + x, 143 + y, (100 * EY) + x, 20 + y, (45 * EY) + x, 60 + y);
                O.moveTo((45 * EY) + x, 60 + y);
                O.curveTo((40 * EY) + x, 65 + y, (35 * EY) + x, 85 + y, (40 * EY) + x, 100 + y);
            } else {
                O.moveTo((40 * EY) + x, 143 + y);
                O.curveTo((0 * EY) + x, 143 + y, (0 * EY) + x, 55 + y, (25 * EY) + x, 55 + y);
                O.moveTo((40 * EY) + x, 143 + y);
                O.curveTo((80 * EY) + x, 143 + y, (80 * EY) + x, 20 + y, (25 * EY) + x, 60 + y);
                O.moveTo((25 * EY) + x, 60 + y);
                O.curveTo((20 * EY) + x, 65 + y, (15 * EY) + x, 85 + y, (20 * EY) + x, 100 + y);
            }
            if (subrayado == true) {
                O.moveTo((0 * EY) + x, 153 + y);
                O.lineTo((90 * EY) + x, 153 + y);
            }
            if (tilde == true) {
                O.moveTo((40 * EY) + x, 45 + y);
                O.lineTo((60 * EY) + x, 25 + y);
                if (bandera == true) {
                    g2.fillOval((40 * EY) + x, 45 + y, 5, 5);
                    g2.fillOval((60 * EY) + x, 25 + y, 5, 5);
                }
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((40 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((0 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((0 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((25 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((80 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((80 * EY) + x, 20 + y, 5, 5);
            g2.fillOval((25 * EY) + x, 60 + y, 5, 5);
            g2.fillOval((25 * EY) + x, 60 + y, 5, 5);
            g2.fillOval((20 * EY) + x, 65 + y, 5, 5);
            g2.fillOval((15 * EY) + x, 85 + y, 5, 5);
            g2.fillOval((20 * EY) + x, 100 + y, 5, 5);
        }
        g2.draw(O);
    }

    public void dibujarP(Graphics2D g2, int x, int y, boolean bandera, boolean negrita, boolean subrayado, boolean cursiva, boolean EspejoY) {
        Path2D.Double P = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                P.moveTo((30 * EY) + x, 80 + y);
                P.curveTo((20 * EY) + x, 120 + y, (15 * EY) + x, 143 + y, (10 * EY) + x, 143 + y);
                P.moveTo((10 * EY) + x, 143 + y);
                P.curveTo((0 * EY) + x, 143 + y, (0 * EY) + x, 80 + y, (20 * EY) + x, 70 + y);
                P.moveTo((20 * EY) + x, 70 + y);
                P.curveTo((22 * EY) + x, 65 + y, (60 * EY) + x, 50 + y, (70 * EY) + x, 55 + y);
                P.moveTo((70 * EY) + x, 55 + y);
                P.curveTo((90 * EY) + x, 65 + y, (60 * EY) + x, 100 + y, (20 * EY) + x, 100 + y);
            } else {
                P.moveTo((20 * EY) + x, 80 + y);
                P.curveTo((20 * EY) + x, 120 + y, (15 * EY) + x, 143 + y, (10 * EY) + x, 143 + y);
                P.moveTo((10 * EY) + x, 143 + y);
                P.curveTo((0 * EY) + x, 143 + y, (0 * EY) + x, 80 + y, (10 * EY) + x, 70 + y);
                P.moveTo((10 * EY) + x, 70 + y);
                P.curveTo((12 * EY) + x, 65 + y, (40 * EY) + x, 50 + y, (60 * EY) + x, 55 + y);
                P.moveTo((60 * EY) + x, 55 + y);
                P.curveTo((90 * EY) + x, 65 + y, (60 * EY) + x, 100 + y, (20 * EY) + x, 100 + y);
            }
            if (subrayado == true) {
                P.moveTo((0 * EY) + x, 153 + y);
                P.lineTo((90 * EY) + x, 153 + y);
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((20 * EY) + x, 80 + y, 5, 5);
            g2.fillOval((20 * EY) + x, 120 + y, 5, 5);
            g2.fillOval((15 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((10 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((0 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((0 * EY) + x, 80 + y, 5, 5);
            g2.fillOval((10 * EY) + x, 70 + y, 5, 5);
            g2.fillOval((12 * EY) + x, 65 + y, 5, 5);
            g2.fillOval((40 * EY) + x, 50 + y, 5, 5);
            g2.fillOval((60 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((90 * EY) + x, 65 + y, 5, 5);
            g2.fillOval((60 * EY) + x, 100 + y, 5, 5);
            g2.fillOval((20 * EY) + x, 100 + y, 5, 5);
        }
        g2.draw(P);
    }

    public void dibujarQ(Graphics2D g2, int x, int y, boolean bandera, boolean negrita, boolean subrayado, boolean cursiva, boolean EspejoY) {
        Path2D.Double Q = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                Q.moveTo((40 * EY) + x, 130 + y);
                Q.curveTo((0 * EY) + x, 143 + y, (20 * EY) + x, 55 + y, (60 * EY) + x, 55 + y);
                Q.moveTo((40 * EY) + x, 143 + y);
                Q.curveTo((80 * EY) + x, 143 + y, (100 * EY) + x, 55 + y, (60 * EY) + x, 55 + y);
                Q.moveTo((40 * EY) + x, 143 + y);
                Q.curveTo((40 * EY) + x, 120 + y, (80 * EY) + x, 143 + y, (80 * EY) + x, 140 + y);
            } else {
                Q.moveTo((40 * EY) + x, 130 + y);
                Q.curveTo((0 * EY) + x, 143 + y, (0 * EY) + x, 55 + y, (40 * EY) + x, 55 + y);
                Q.moveTo((40 * EY) + x, 143 + y);
                Q.curveTo((80 * EY) + x, 143 + y, (80 * EY) + x, 55 + y, (40 * EY) + x, 55 + y);
                Q.moveTo((40 * EY) + x, 143 + y);
                Q.curveTo((40 * EY) + x, 120 + y, (80 * EY) + x, 143 + y, (80 * EY) + x, 140 + y);
            }
            if (subrayado == true) {
                Q.moveTo((0 * EY) + x, 153 + y);
                Q.lineTo((100 * EY) + x, 153 + y);
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((40 * EY) + x, 130 + y, 5, 5);
            g2.fillOval((0 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((0 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((40 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((40 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((80 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((80 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((40 * EY) + x, 120 + y, 5, 5);
            g2.fillOval((80 * EY) + x, 140 + y, 5, 5);
        }
        g2.draw(Q);
    }

    public void dibujarR(Graphics2D g2, int x, int y, boolean bandera, boolean negrita, boolean subrayado, boolean cursiva, boolean EspejoY) {
        Path2D.Double R = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                R.moveTo((30 * EY) + x, 80 + y);
                R.curveTo((20 * EY) + x, 120 + y, (15 * EY) + x, 143 + y, (10 * EY) + x, 143 + y);
                R.moveTo((10 * EY) + x, 143 + y);
                R.curveTo((0 * EY) + x, 143 + y, (0 * EY) + x, 80 + y, (20 * EY) + x, 70 + y);
                R.moveTo((20 * EY) + x, 70 + y);
                R.curveTo((22 * EY) + x, 65 + y, (60 * EY) + x, 50 + y, (70 * EY) + x, 55 + y);
                R.moveTo((70 * EY) + x, 55 + y);
                R.curveTo((90 * EY) + x, 65 + y, (60 * EY) + x, 100 + y, (30 * EY) + x, 100 + y);
                R.moveTo((30 * EY) + x, 100 + y);
                R.curveTo((40 * EY) + x, 100 + y, (50 * EY) + x, 143 + y, (70 * EY) + x, 143 + y);
            } else {
                R.moveTo((20 * EY) + x, 80 + y);
                R.curveTo((20 * EY) + x, 120 + y, (15 * EY) + x, 143 + y, (10 * EY) + x, 143 + y);
                R.moveTo((10 * EY) + x, 143 + y);
                R.curveTo((0 * EY) + x, 143 + y, (0 * EY) + x, 80 + y, (10 * EY) + x, 70 + y);
                R.moveTo((10 * EY) + x, 70 + y);
                R.curveTo((12 * EY) + x, 65 + y, (40 * EY) + x, 50 + y, (60 * EY) + x, 55 + y);
                R.moveTo((60 * EY) + x, 55 + y);
                R.curveTo((90 * EY) + x, 65 + y, (60 * EY) + x, 100 + y, (20 * EY) + x, 100 + y);
                R.moveTo((20 * EY) + x, 100 + y);
                R.curveTo((40 * EY) + x, 100 + y, (50 * EY) + x, 143 + y, (70 * EY) + x, 143 + y);
            }
            if (subrayado == true) {
                R.moveTo((0 * EY) + x, 153 + y);
                R.lineTo((80 * EY) + x, 153 + y);
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((20 * EY) + x, 80 + y, 5, 5);
            g2.fillOval((20 * EY) + x, 120 + y, 5, 5);
            g2.fillOval((15 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((10 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((0 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((0 * EY) + x, 80 + y, 5, 5);
            g2.fillOval((10 * EY) + x, 70 + y, 5, 5);
            g2.fillOval((12 * EY) + x, 65 + y, 5, 5);
            g2.fillOval((40 * EY) + x, 50 + y, 5, 5);
            g2.fillOval((60 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((60 * EY) + x, 55 + y, 5, 5);
            g2.fillOval((90 * EY) + x, 65 + y, 5, 5);
            g2.fillOval((60 * EY) + x, 100 + y, 5, 5);
            g2.fillOval((20 * EY) + x, 100 + y, 5, 5);
            g2.fillOval((40 * EY) + x, 100 + y, 5, 5);
            g2.fillOval((50 * EY) + x, 143 + y, 5, 5);
            g2.fillOval((70 * EY) + x, 143 + y, 5, 5);
        }
        g2.draw(R);
    }

    public void dibujarS(Graphics2D g2, int x, int y, boolean bandera, boolean negrita, boolean subrayado, boolean cursiva, boolean EspejoY) {
        Path2D.Double S = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                S.moveTo((40 * EY) + x, 65 + y);
                S.curveTo((20 * EY) + x, 85 + y, (70 * EY) + x, 100 + y, (70 * EY) + x, 120 + y);
                S.moveTo((40 * EY) + x, 65 + y);
                S.curveTo((42 * EY) + x, 50 + y, (110 * EY) + x, 50 + y, (105 * EY) + x, 70 + y);
                S.moveTo((70 * EY) + x, 120 + y);
                S.curveTo((68 * EY) + x, 135 + y, (50 * EY) + x, 160 + y, (10 * EY) + x, 130 + y);
            } else {
                S.moveTo((10*EY) + x, 65 + y);
                S.curveTo((0*EY) + x, 85 + y, (70*EY) + x, 100 + y, (70*EY) + x, 120 + y);
                S.moveTo((10*EY) + x, 65 + y);
                S.curveTo((12*EY) + x, 50 + y, (80*EY) + x, 50 + y, (75*EY) + x, 70 + y);
                S.moveTo((70*EY) + x, 120 + y);
                S.curveTo((68*EY) + x, 135 + y, (50*EY) + x, 160 + y, (10*EY) + x, 130 + y);
            }
            if (subrayado == true) {
                S.moveTo((0 *EY)+ x, 153 + y);
                S.lineTo((80*EY) + x, 153 + y);
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((10*EY) + x, 65 + y, 5, 5);
            g2.fillOval((0*EY) + x, 85 + y, 5, 5);
            g2.fillOval((70*EY) + x, 100 + y, 5, 5);
            g2.fillOval((70*EY) + x, 120 + y, 5, 5);
            g2.fillOval((12*EY) + x, 50 + y, 5, 5);
            g2.fillOval((80*EY) + x, 50 + y, 5, 5);
            g2.fillOval((75*EY) + x, 70 + y, 5, 5);
            g2.fillOval((68*EY) + x, 135 + y, 5, 5);
            g2.fillOval((50*EY) + x, 160 + y, 5, 5);
            g2.fillOval((10*EY) + x, 130 + y, 5, 5);
        }
        g2.draw(S);
    }

    public void dibujarT(Graphics2D g2, int x, int y, boolean bandera, boolean negrita, boolean subrayado, boolean cursiva, boolean EspejoY) {
        Path2D.Double T = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                T.moveTo((35*EY) + x, 60 + y);
                T.curveTo((0*EY) + x, 60 + y, (0*EY) + x, 20 + y, (70*EY) + x, 55 + y);
                T.moveTo((70*EY) + x, 55 + y);
                T.curveTo((90*EY) + x, 65 + y, (120*EY) + x, 50 + y, (90*EY) + x, 50 + y);
                T.moveTo((90*EY) + x, 50 + y);
                T.curveTo((50*EY) + x, 45 + y, (60*EY) + x, 120 + y, (50*EY) + x, 143 + y);
            } else {
                T.moveTo((35*EY) + x, 70 + y);
                T.curveTo((0*EY) + x, 75 + y, (0*EY) + x, 40 + y, (70*EY) + x, 65 + y);
                T.moveTo((70*EY) + x, 65 + y);
                T.curveTo((90*EY) + x, 60 + y, (120*EY) + x, 60 + y, (90*EY) + x, 55 + y);
                T.moveTo((90*EY) + x, 55 + y);
                T.curveTo((50*EY) + x, 55 + y, (60*EY) + x, 120 + y, (70*EY) + x, 143 + y);
            }
            if (subrayado == true) {
                T.moveTo((0*EY) + x, 153 + y);
                T.lineTo((100*EY) + x, 153 + y);
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((35*EY) + x, 70 + y, 5, 5);
            g2.fillOval((0*EY) + x, 75 + y, 5, 5);
            g2.fillOval((0*EY) + x, 40 + y, 5, 5);
            g2.fillOval((70*EY) + x, 65 + y, 5, 5);
            g2.fillOval((90*EY) + x, 60 + y, 5, 5);
            g2.fillOval((120*EY) + x, 60 + y, 5, 5);
            g2.fillOval((90*EY) + x, 55 + y, 5, 5);
            g2.fillOval((50*EY) + x, 55 + y, 5, 5);
            g2.fillOval((60*EY) + x, 120 + y, 5, 5);
            g2.fillOval((70*EY) + x, 143 + y, 5, 5);
        }
        g2.draw(T);
    }

    public void dibujarU(Graphics2D g2, int x, int y, boolean bandera, boolean tilde, boolean negrita, boolean subrayado, boolean cursiva, boolean EspejoY) {
        Path2D.Double U = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                U.moveTo((45*EY) + x, 75 + y);
                U.curveTo((20*EY) + x, 85 + y, (60*EY) + x, 15 + y, (50*EY) + x, 100 + y);
                U.moveTo((50*EY) + x, 100 + y);
                U.curveTo((50*EY) + x, 200 + y, (85*EY) + x, 100 + y, (100*EY) + x, 70 + y);
                U.moveTo((100*EY) + x, 70 + y);
                U.curveTo((100*EY) + x, 55 + y, (90*EY) + x, 40 + y, (90*EY) + x, 120 + y);
                U.moveTo((90*EY) + x, 120 + y);
                U.curveTo((90*EY) + x, 120 + y, (90*EY) + x, 143 + y, (100*EY) + x, 143 + y);
            } else {
                U.moveTo((25*EY) + x, 75 + y);
                U.curveTo((0*EY) + x, 85 + y, (40*EY) + x, 15 + y, (40*EY) + x, 100 + y);
                U.moveTo((40*EY) + x, 100 + y);
                U.curveTo((50*EY) + x, 200 + y, (85*EY) + x, 100 + y, (80*EY) + x, 70 + y);
                U.moveTo((80*EY) + x, 70 + y);
                U.curveTo((80*EY) + x, 55 + y, (75*EY) + x, 40 + y, (75*EY) + x, 120 + y);
                U.moveTo((75*EY) + x, 120 + y);
                U.curveTo((75*EY) + x, 120 + y, (80*EY) + x, 143 + y, (90*EY) + x, 143 + y);
            }
            if (subrayado == true) {
                U.moveTo((0*EY) + x, 153 + y);
                U.lineTo((100*EY) + x, 153 + y);
            }
            if (tilde == true) {
                U.moveTo((60*EY) + x, 50 + y);
                U.lineTo((80*EY) + x, 30 + y);
                if (bandera == true) {
                    g2.fillOval((60*EY) + x, 50 + y, 5, 5);
                    g2.fillOval((80*EY) + x, 30 + y, 5, 5);
                }
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((25*EY) + x, 75 + y, 5, 5);
            g2.fillOval((0*EY) + x, 85 + y, 5, 5);
            g2.fillOval((40*EY) + x, 15 + y, 5, 5);
            g2.fillOval((40*EY) + x, 100 + y, 5, 5);
            g2.fillOval((50*EY) + x, 200 + y, 5, 5);
            g2.fillOval((85*EY) + x, 100 + y, 5, 5);
            g2.fillOval((80*EY) + x, 70 + y, 5, 5);
            g2.fillOval((80*EY) + x, 55 + y, 5, 5);
            g2.fillOval((75*EY) + x, 40 + y, 5, 5);
            g2.fillOval((75*EY) + x, 120 + y, 5, 5);
            g2.fillOval((80*EY) + x, 143 + y, 5, 5);
            g2.fillOval((90*EY) + x, 143 + y, 5, 5);
        }
        g2.draw(U);
    }

    public void dibujarV(Graphics2D g2, int x, int y, boolean bandera, boolean negrita, boolean subrayado, boolean cursiva, boolean EspejoY) {
        Path2D.Double V = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                V.moveTo((25*EY) + x, 75 + y);
                V.curveTo((0*EY) + x, 85 + y, (40*EY) + x, 15 + y, (40*EY) + x, 100 + y);
                V.moveTo((40*EY) + x, 100 + y);
                V.curveTo((50*EY) + x, 200 + y, (85*EY) + x, 100 + y, (95*EY) + x, 55 + y);
            } else {
                V.moveTo((25*EY) + x, 75 + y);
                V.curveTo((0*EY) + x, 85 + y, (40*EY) + x, 15 + y, (40*EY) + x, 100 + y);
                V.moveTo((40*EY) + x, 100 + y);
                V.curveTo((50*EY) + x, 200 + y, (80*EY) + x, 100 + y, (80*EY) + x, 55 + y);
            }
            if (subrayado == true) {
                V.moveTo((0*EY) + x, 153 + y);
                V.lineTo((90*EY) + x, 153 + y);
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((25*EY) + x, 75 + y, 5, 5);
            g2.fillOval((0*EY) + x, 85 + y, 5, 5);
            g2.fillOval((40*EY) + x, 15 + y, 5, 5);
            g2.fillOval((40*EY) + x, 100 + y, 5, 5);
            g2.fillOval((50*EY) + x, 200 + y, 5, 5);
            g2.fillOval((80*EY) + x, 100 + y, 5, 5);
            g2.fillOval((80*EY) + x, 55 + y, 5, 5);
        }
        g2.draw(V);
    }

    public void dibujarW(Graphics2D g2, int x, int y, boolean bandera, boolean negrita, boolean subrayado, boolean cursiva, boolean EspejoY) {
        Path2D.Double W = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                W.moveTo((25*EY) + x, 75 + y);
                W.curveTo((0 *EY)+ x, 85 + y, (40*EY) + x, 15 + y, (40*EY) + x, 100 + y);
                W.moveTo((40*EY) + x, 100 + y);
                W.curveTo((40*EY) + x, 190 + y, (60*EY) + x, 120 + y, (60*EY) + x, 100 + y);
                W.moveTo((60*EY) + x, 100 + y);
                W.curveTo((60*EY) + x, 225 + y, (70*EY) + x, 80 + y, (90*EY) + x, 70 + y);
            } else {
                W.moveTo((25*EY) + x, 75 + y);
                W.curveTo((0 *EY)+ x, 85 + y, (40*EY) + x, 15 + y, (40*EY) + x, 100 + y);
                W.moveTo((40*EY) + x, 100 + y);
                W.curveTo((40*EY) + x, 190 + y, (60*EY) + x, 120 + y, (60*EY) + x, 100 + y);
                W.moveTo((60*EY) + x, 100 + y);
                W.curveTo((60*EY) + x, 225 + y, (100*EY) + x, 35 + y, (80*EY) + x, 70 + y);
            }
            if (subrayado == true) {
                W.moveTo((0*EY) + x, 153 + y);
                W.lineTo((120*EY) + x, 153 + y);
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((25*EY) + x, 75 + y, 5, 5);
            g2.fillOval((0 *EY)+ x, 85 + y, 5, 5);
            g2.fillOval((40*EY) + x, 15 + y, 5, 5);
            g2.fillOval((40*EY) + x, 100 + y, 5, 5);
            g2.fillOval((40*EY) + x, 190 + y, 5, 5);
            g2.fillOval((60*EY) + x, 120 + y, 5, 5);
            g2.fillOval((60*EY) + x, 100 + y, 5, 5);
            g2.fillOval((60*EY) + x, 225 + y, 5, 5);
            g2.fillOval((100*EY) + x, 35 + y, 5, 5);
            g2.fillOval((80*EY) + x, 70 + y, 5, 5);
        }
        g2.draw(W);
    }

    public void dibujarX(Graphics2D g2, int x, int y, boolean bandera, boolean negrita, boolean subrayado, boolean cursiva, boolean EspejoY) {
        Path2D.Double X = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                X.moveTo((10*EY) + x, 60 + y);
                X.curveTo((40*EY) + x, 20 + y, (70*EY) + x, 190 + y, (100*EY) + x, 130 + y);
                X.moveTo((0*EY) + x, 135 + y);
                X.curveTo((10*EY) + x, 180 + y, (90*EY) + x, 20 + y, (100*EY) + x, 60 + y);
            } else {
                X.moveTo((0*EY) + x, 60 + y);
                X.curveTo((30*EY) + x, 20 + y, (70*EY) + x, 190 + y, (100*EY) + x, 130 + y);
                X.moveTo((0*EY)+ x, 135 + y);
                X.curveTo((30*EY) + x, 180 + y, (70*EY) + x, 20 + y, (100*EY) + x, 60 + y);
            }
            if (subrayado == true) {
                X.moveTo((0*EY) + x, 153 + y);
                X.lineTo((110*EY) + x, 153 + y);
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((0*EY) + x, 60 + y, 5, 5);
            g2.fillOval((30*EY) + x, 20 + y, 5, 5);
            g2.fillOval((70*EY) + x, 190 + y, 5, 5);
            g2.fillOval((100*EY) + x, 130 + y, 5, 5);
            g2.fillOval((0*EY) + x, 135 + y, 5, 5);
            g2.fillOval((30*EY) + x, 180 + y, 5, 5);
            g2.fillOval((70*EY) + x, 20 + y, 5, 5);
            g2.fillOval((100*EY) + x, 60 + y, 5, 5);
        }
        g2.draw(X);
    }

    public void dibujarY(Graphics2D g2, int x, int y, boolean bandera, boolean negrita, boolean subrayado, boolean cursiva, boolean EspejoY) {
        Path2D.Double Y = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                Y.moveTo((25*EY) + x, 75 + y);
                Y.curveTo((0*EY) + x, 85 + y, (40*EY) + x, 15 + y, (40*EY) + x, 100 + y);
                Y.moveTo((40*EY) + x, 100 + y);
                Y.curveTo((50*EY) + x, 110 + y, (60*EY) + x, 100 + y, (70*EY) + x, 60 + y);
                Y.moveTo((70 *EY)+ x, 60 + y);
                Y.curveTo((70*EY) + x, 45 + y, (55*EY) + x, 70 + y, (55*EY) + x, 140 + y);
                Y.moveTo((55*EY) + x, 140 + y);
                Y.curveTo((55*EY) + x, 170 + y, (25*EY) + x, 200 + y, (30*EY) + x, 160 + y);
                Y.moveTo((30*EY) + x, 160 + y);
                Y.curveTo((40*EY) + x, 120 + y, (55*EY) + x, 110 + y, (60*EY) + x, 115 + y);
                Y.moveTo((60*EY) + x, 115 + y);
                Y.curveTo((70*EY) + x, 115 + y, (80*EY) + x, 143 + y, (100*EY) + x, 143 + y);
            } else {
                Y.moveTo((25*EY) + x, 75 + y);
                Y.curveTo((0*EY) + x, 85 + y, (40*EY) + x, 15 + y, (40*EY) + x, 100 + y);
                Y.moveTo((40*EY) + x, 100 + y);
                Y.curveTo((40*EY) + x, 110 + y, (70*EY) + x, 100 + y, (70*EY) + x, 60 + y);
                Y.moveTo((70*EY) + x, 60 + y);
                Y.curveTo((70*EY) + x, 45 + y, (65*EY) + x, 70 + y, (65*EY) + x, 140 + y);
                Y.moveTo((65*EY) + x, 140 + y);
                Y.curveTo((65*EY) + x, 170 + y, (35*EY) + x, 200 + y, (40*EY) + x, 160 + y);
                Y.moveTo((40*EY) + x, 160 + y);
                Y.curveTo((50*EY) + x, 120 + y, (65*EY) + x, 110 + y, (70*EY) + x, 115 + y);
                Y.moveTo((70*EY) + x, 115 + y);
                Y.curveTo((70*EY) + x, 115 + y, (80*EY) + x, 143 + y, (100*EY) + x, 143 + y);
            }
            if (subrayado == true) {
                Y.moveTo((0 *EY)+ x, 153 + y);
                Y.lineTo((110*EY) + x, 153 + y);
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((25*EY) + x, 75 + y, 5, 5);
            g2.fillOval((0 *EY)+ x, 85 + y, 5, 5);
            g2.fillOval((40*EY) + x, 15 + y, 5, 5);
            g2.fillOval((40*EY) + x, 100 + y, 5, 5);
            g2.fillOval((40*EY) + x, 110 + y, 5, 5);
            g2.fillOval((70*EY) + x, 100 + y, 5, 5);
            g2.fillOval((70*EY) + x, 60 + y, 5, 5);
            g2.fillOval((70*EY) + x, 45 + y, 5, 5);
            g2.fillOval((65*EY) + x, 70 + y, 5, 5);
            g2.fillOval((65*EY) + x, 140 + y, 5, 5);
            g2.fillOval((65*EY) + x, 170 + y, 5, 5);
            g2.fillOval((35*EY) + x, 200 + y, 5, 5);
            g2.fillOval((40*EY) + x, 160 + y, 5, 5);
            g2.fillOval((50*EY) + x, 120 + y, 5, 5);
            g2.fillOval((65*EY) + x, 110 + y, 5, 5);
            g2.fillOval((70*EY) + x, 115 + y, 5, 5);
            g2.fillOval((80*EY) + x, 143 + y, 5, 5);
            g2.fillOval((100*EY) + x, 143 + y, 5, 5);
        }
        g2.draw(Y);
    }

    public void dibujarZ(Graphics2D g2, int x, int y, boolean bandera, boolean negrita, boolean subrayado, boolean cursiva, boolean EspejoY) {
        Path2D.Double Z = new Path2D.Double();
        int limite = 1;
        int EY = 1;
        if (negrita == true) {
            limite = 5;
        }
        if (EspejoY) {
            EY = -1;
        }
        for (int i = 0; i < limite; i++) {
            if (cursiva == true) {
                Z.moveTo((40*EY) + x, 80 + y);
                Z.curveTo((10*EY) + x, 70 + y, 10 + x, 55 + y, 110 + x, 60 + y);
                Z.curveTo((90*EY) + x, 20 + y, 30 + x, 143 + y, 0 + x, 135 + y);
                Z.curveTo((10*EY) + x, 110 + y, 80 + x, 175 + y, 100 + x, 120 + y);
                Z.moveTo((30*EY) + x, 100 + y);
                Z.lineTo((70*EY) + x, 100 + y);
            } else {
                Z.moveTo((30*EY) + x, 80 + y);
                Z.curveTo((0*EY) + x, 70 + y, (0*EY) + x, 55 + y, (100*EY) + x, 60 + y);
                Z.curveTo((90*EY) + x, 20 + y, (30*EY) + x, 143 + y, (0*EY) + x, 143 + y);
                Z.curveTo((10*EY) + x, 110 + y, (90*EY) + x, 175 + y, (100*EY) + x, 120 + y);
                Z.moveTo((30*EY) + x, 100 + y);
                Z.lineTo((70*EY) + x, 100 + y);
            }
            if (subrayado == true) {
                Z.moveTo((0*EY) + x, 153 + y);
                Z.lineTo((110*EY) + x, 153 + y);
            }
            x += 1;
        }
        if (bandera == true) {
            g2.fillOval((30*EY) + x, 80 + y, 5, 5);
            g2.fillOval((0*EY) + x, 70 + y, 5, 5);
            g2.fillOval((0*EY) + x, 55 + y, 5, 5);
            g2.fillOval((100*EY) + x, 60 + y, 5, 5);
            g2.fillOval((90*EY) + x, 20 + y, 5, 5);
            g2.fillOval((30*EY) + x, 143 + y, 5, 5);
            g2.fillOval((0*EY) + x, 143 + y, 5, 5);
            g2.fillOval((10*EY) + x, 110 + y, 5, 5);
            g2.fillOval((90*EY) + x, 175 + y, 5, 5);
            g2.fillOval((100*EY) + x, 120 + y, 5, 5);
            g2.fillOval((30*EY) + x, 100 + y, 5, 5);
            g2.fillOval((70*EY) + x, 100 + y, 5, 5);
        }
        g2.draw(Z);
    }
}
